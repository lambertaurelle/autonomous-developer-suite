...
> **Why this file exists.** `/tdd-build` carries the *method* (TDD, OO/Modular design, clean docs).
> This file carries the *layout* — where code goes — so the structure is
> **deterministic** rather than improvised on each run. `/tdd-build` MUST read this
...
```
<repo-root>/
├── SPEC.md                  # the contract /tdd-build obeys — ROOT, load-bearing (hooks expect it here)
├── <package manifest file>  # project configuration (e.g., package.json, pyproject.toml, Cargo.toml, go.mod)
...
└── <CODE_ROOT>/             # source root (e.g. src/)
   └── <package>/            # the importable package/module (= the built artifact)
...
- **pure core** (`<PURE_DIR>`) — no I/O, no network, no model calls; same input →
  same output. This is what `/tdd-build` builds and tests in TDD.
...
- **One rule class/module per file**, named per `<RULE_FILE_PATTERN>` (e.g. `r2_<slug>.<ext>`),
  under `<RULES_DIR>`.
- Each rule **implements the `Rule` interface / abstract contract** (e.g.
  `evaluate(...) -> Decision | null`; `null` = "I don't apply"). Explicit contract implementation — not a bare dynamic type — is deliberate: it lets the **engine enforce the contract** (a rule that omits the method cannot be instantiated or compiled). Small single-purpose rule classes/modules are intentional; configure the project linter once centrally to allow them (e.g. `pyproject.toml`, `.eslintrc.json`, `golangci.yml`), never per-file inline disables.
- The **engine** (`<engine module>`) is a **first-class artifact**: it holds the rule
  ...
  the SPEC names, and returns the first non-`null` decision; the last rule is the
...
- Live in `<TESTS_DIR>`, **mirroring** the rule files: `<TEST_FILE_PREFIX><rule>.<ext>`.
...
The values below live in `code-layout.env`; hooks read them and block
...
- New rule files live under `<RULES_DIR>` matching `<RULE_FILE_PATTERN>` and have a
  matching `<TESTS_DIR>/<TEST_FILE_PREFIX><rule>.<ext>`.
