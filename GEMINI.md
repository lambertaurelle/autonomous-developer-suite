# **Project Constitution & Antigravity CLI Guidelines: GEMINI.md**
This document serves as the supreme governance charter and architectural constitution for all AI agents and sub-agents executing operations within this repository via **Antigravity CLI (`agy`)**. All background tasks, TDD cycles, and autonomous development pipelines must strictly adhere to the rules, cascades, and isolation protocols detailed below.

> [!NOTE]
> This file (`GEMINI.md`) is automatically loaded into the agent context by **Antigravity CLI (`agy`)**. It mirrors [AGENTS.md](file:///home/oph/Documents/antigravity/goofy-chandrasekhar/AGENTS.md) for 100% dual compatibility across CLI and IDE surfaces.
...
- **Skills (Method)**: Located under `.gemini/skills/`, these describe procedures, guidelines, and workflows carrying logical judgment. They suggest the path but cannot guarantee execution correctness. In `agy`, all slash commands (including `/interview-me` and `/implementation-loop`) are exposed via `.gemini/skills/<skill-name>/SKILL.md`.
- **Hooks (Enforcement)**: Located under `scripts/hooks/` and registered in `.gemini/hooks.json` and `.git/hooks/pre-commit`, these are zero-token, absolute-compliance gates executing outside the LLM context. Any non-zero exit code blocks execution immediately.
...
- **Gemini 3.6 Flash**: Model for deep reasoning and complex coding tasks (`/interview-prd`, `/arch-gate`, `/specify`, `/tdd-build`, `/e2e-test`, `/interview-me`, `/implementation-loop`), architectural drafting, TDD cycle debugging, complex refactoring, and interactive interviews.
...
- **Repository Setup (`init-project.sh`)**: Checks if `gh` is installed, verifies auth status, and interactively prompts to create a public/private GitHub repository if no 'origin' remote exists.
- **Backlog Generation (`/prd2backlog`)**: All backlog items extracted from sealed `docs/PRD.md` are automatically created or reconciled as GitHub Issues via native `gh` CLI (or `github` MCP server). The assigned GitHub Issue ID (e.g., `#123`) is dynamically parsed and saved to metadata.
- **Autonomous Specification Drafting (`/specify`)**: Pulls user story criteria from GitHub via `gh` CLI, drafts `SPEC.md` at repository root, validates automatically via `/review-spec` sub-agent audit (zero HITL pauses), creates `issue/<number>-<title>` branch, commits `SPEC.md`, transitions GitHub issue stage, and hands off to `/tdd-build`.
